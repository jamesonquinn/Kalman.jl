...
somenice = arunmeans[model %in% c("finkel","franken")&
                     ...
                     ,]
dim(arunmeans[abs(meankl)>10,])/dim(arunmeans)
...
#####MONEY GRAPH
ggplot(data=somenice[model=="finkel",],aes(x=steps,y=meankl)
) + geom_path(aes(linetype=params,color=params)
) + geom_point(aes(color=params)
) + theme_bw() + scale_color_uchicago() + labs(y="mean KL divergence per locus",
                                               color="History sampling weights",
                                               linetype="History sampling weights")
